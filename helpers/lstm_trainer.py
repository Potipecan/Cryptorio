...
def train(dataset, output_filename):
    ...
    plt.plot(dates_y, test_y[:, 0], label='actual_high')
    plt.plot(dates_y, test_y[:, 1], label='actual_low')  
    plt.plot(dates_y, pred_y[:, 0], label='pred_high')
    plt.plot(dates_y, pred_y[:, 1], label='pred_low')
